...
description="Informs Nagios about last AIDE log result."
...
## Check sanity
[[ -z $1 ]] && usage 3
[[ ! -d ${home_dir}/clients/${1} ]] && { echo "ERROR client does not exist." && exit 3; }
## Check whether client has logs
is_empty=(${clients_log}/*)
(( ${#is_empty[@]} )) || { echo "ERROR client does not have logs." && exit 3; }
## Script body
...
if (( ($(date +%s) - ${last_log##*-}) > 86399 )); then
	echo "outdated logs, the last log file is older than 24h." && exit 2
fi	
if grep "found differences between" ${last_log} > /dev/null; then
	added=$(sed -re '/Added files:/!d' -e 's|[^0-9]+||g' ${last_log})
	removed=$(sed -re '/Removed files:/!d' -e 's|[^0-9]+||g' ${last_log})
	changed=$(sed -re '/Changed files:/!d' -e 's|[^0-9]+||g' ${last_log})
